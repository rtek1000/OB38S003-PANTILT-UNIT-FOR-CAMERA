...
uint8_t timer0_cnt1 = 0;
uint8_t timer0_cnt2 = 0;

extern uint8_t mot1_enabled;
extern uint8_t mot1_direction;
extern uint8_t mot1_speed_tmr;

extern uint8_t mot2_enabled;
extern uint8_t mot2_direction;
extern uint8_t mot2_speed_tmr;

extern uint8_t timeout_receiv;
extern uint8_t buffer_index2;
...
//	}
	...
	if (timer0_cnt1 < mot1_speed_tmr) {
		timer0_cnt1++;
	} else {
		timer0_cnt1 = 0;

		if (mot1_direction == MOTOR_CC) {
			if (mot1_enabled == true) {
				motor1_step_cc();
			}
		} else {
			if (mot1_enabled == true) {
	...
	if (timer0_cnt2 < mot2_speed_tmr) {
		timer0_cnt2++;
	} else {
		timer0_cnt2 = 0;

		if (mot2_direction == MOTOR_CC) {
			if (mot2_enabled == true) {
				motor2_step_cc();
			}
		} else {
			if (mot2_enabled == true) {
