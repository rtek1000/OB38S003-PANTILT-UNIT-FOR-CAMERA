...
void init_port_pins(void) {
	...
	// MOT1EN push pull (P1.5)
	P1M1 &= ~0x20;
	P1M0 |= 0x20;

	// MOT1PA push pull (P1.6)
	P1M1 &= ~0x40;
	P1M0 |= 0x40;

	// MOT1PB push pull (P1.7)
	P1M1 &= ~0x80;
	P1M0 |= 0x80;

//	// MOT2EN push pull (P0.4)
//	P0M1 &= ~0x10;
//	P0M0 |= 0x10;

	// MOT2PA push pull (P0.5)
	P0M1 &= ~0x20;
	P0M0 |= 0x20;

	// MOT2PB push pull (P0.7)
	P0M1 &= ~0x80;
	P0M0 |= 0x80;
...
void init_uart(void) {
	...
	// mode 1, no parity bit SM0 = 0 and SM2 = 0 by default for 8N1 uart
	SM1 = 1;
